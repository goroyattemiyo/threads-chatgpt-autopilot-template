...
6. 短期アクセストークン生成時の応答に`user_id`が表示される場合は、その値を控える
7. 短期アクセストークンを長期アクセストークンへ交換する
8. Meta公式のアクセストークンデバッガーで、長期アクセストークンのUser ID、対象アプリ、有効期限、権限を確認する
9. 短期トークン生成時の`user_id`とデバッガーのUser IDが両方確認できる場合は一致を確認する
10. 利用者本人が長期アクセストークンを`THREADS_ACCESS_TOKEN`へ登録する
11. 利用者本人が確認済みのThreadsユーザーIDを`THREADS_USER_ID`へ登録する
12. `Test Threads Connection`を実行し、取得されたプロフィールのID・ユーザー名がアカウントBと一致することを確認する

アクセストークンデバッガーで確認できるUser IDは取得手段として利用できますが、デバッガー表示だけで最終判断しません。プロフィール取得結果のユーザー名まで確認してください。

パスワード、アクセストークン、認証コードは、運営者やAIへ送信しないでください。アクセストークンデバッガーへの入力も利用者本人がMeta公式画面で行います。
