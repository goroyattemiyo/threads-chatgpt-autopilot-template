...
GitHub Actionsは24時間5分間隔では起動しません。既定では、各投稿予定時刻の3分前、2分後、7分後だけ起動します。

`time_slots`または`schedule_offsets_minutes`をmainへ反映すると、`Sync Posting Schedule`が設定タイムゾーンからUTCへ変換し、`.github/workflows/post-due.yml`のcronを自動同期します。通常の時刻変更ではworkflowを直接編集しません。

GitHub Actionsのschedule実行は遅れる場合があるため、秒単位・分単位の厳密な時刻保証はしません。詳細は`docs/SCHEDULE_WINDOWS.md`を参照してください。
...
画像用設定・接続エラーの復旧は`docs/IMAGE_DELIVERY_RECOVERY.md`を参照してください。

## 初期設定

1. `TEMPLATE_SETUP.md`を読む
2. `config/service.yml`を顧客環境へ合わせる
3. `Sync Posting Schedule`と`Test Posting Schedule Sync`を確認する
4. 利用者本人がGitHub Secretsを登録する
5. 画像投稿時はPublic画像リポジトリとVariablesを設定する
6. ChatGPTまたはClaudeの通常チャットとGitHubを接続する
7. `Test Threads Connection`を実行する
8. dry runで投稿予定を確認する
9. 少数のテスト投稿で動作確認する
10. 利用するチャットに応じて`AGENTS.md`または`CLAUDE.md`を確認する
